...
// ── DRAWER ACCORDION COMPONENT ─────────────────────────────────────────────
// ✅ Drawer-style accordion - uses CSS module for styling
function DrawerAccordion({
  ...
  return (
    <div className={styles.card}>
      {/* ── Accordion Header (Drawer Toggle) ── */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className={styles.accordionHeader}
        style={{
          background: isOpen ? `${accentColor}15` : "#fafafa",
...
// ── Purpose / service-specific sections with drawer accordions ───────────
...
const LegalSection = ({ appt }) => (
  ...
  >
    ...
    <div
      style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #f0f0f0" }}
    >
      <InfoRow label="Case Type" value="Child Custody & Adoption" />
      <InfoRow label="Court Name" value="Family Court - Quezon City" />
      <InfoRow label="Case Number" value="FC-2026-0451" />
      <InfoRow label="Judge Assigned" value="Hon. Maria Santos" />
      <InfoRow label="Hearing Date" value="April 15, 2026" />
      <InfoRow
        label="Psychologist Role"
        value="Expert Witness / Child Assessment"
      />
    </div>
...
const WorkSection = ({ appt }) => (
  ...
  >
    ...
    <div
      style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #f0f0f0" }}
    >
      <InfoRow label="Company" value="Accenture Philippines" />
      <InfoRow label="Position" value="Senior Business Analyst" />
      <InfoRow label="Department" value="Digital Solutions" />
      <InfoRow
        label="Reason for Assessment"
        value="Work Stress & Burnout Evaluation"
      />
      <InfoRow label="Work Duration" value="3 years, 4 months" />
      <InfoRow label="Assessment Requested By" value="HR Department" />
    </div>
    ...
    >
      ...
      <div style={{ fontSize: 11, color: "#666", lineHeight: 1.6 }}>
        <div>• High workload (60+ hours per week)</div>
        <div>• Sleep disturbances and fatigue</div>
        <div>• Difficulty concentrating</div>
        <div>• Anxiety related to deadlines</div>
        <div>• Work-life balance concerns</div>
      </div>
    </div>
  </DrawerAccordion>
);
      ...
      <div className={styles.overlay} onClick={onClose}>
        <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
          ...
          <div className={styles.scrollBody}>
            ...
            {/* ✅ DRAWER ACCORDIONS - Professional Drawer-style */}
