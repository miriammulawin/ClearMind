...
// ── API config — reads from env, same as DoctorAppointment ────────────────
...
/* ─────────────────────────────────────────────────────────
   API fetch — fetches appointments for a specific date
   Backend already scopes by role (Doctor sees only their own)
───────────────────────────────────────────────────────── */
// ✅ AYOS — dalhin lahat ng allowed statuses
async function fetchAppointments(date, signal) {
  const dateStr = format(date, "yyyy-MM-dd");
  const res = await fetch(
    `${API_BASE}/appointments?date=${dateStr}`,  // ← tanggal ang &status=confirmed
  ...
  // ✅ I-filter dito sa client — pareho sa DoctorAppointment
  const allowed = ["confirmed", "completed", "no_show"];
  return raw.filter((r) => allowed.includes((r.status ?? "").toLowerCase()));
}
...
/* ─────────────────────────────────────────────────────────
   ...
   Props:
     ...
     events        — calendar events array (optional, unused — we re-fetch)
     singleEventId — number | null
     onRefresh     — fn (optional) — called after actions that mutate data
───────────────────────────────────────────────────────── */
function DayAppointmentsModal({ isOpen, onClose, selectedDate, onRefresh }) {
