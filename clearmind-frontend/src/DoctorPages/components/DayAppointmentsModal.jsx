...
const purposeInlineStyle = (purpose) => {
  switch (purpose) {
    case PURPOSES.VAWC:
      return { background: "#ede9f6", color: "#4D227C", border: "1px solid #d8ccf0" };
    case PURPOSES.LEGAL:
      return { background: "#fce8e8", color: "#8B4545", border: "1px solid #f5c6c6" };
    case PURPOSES.SCHOOL:
      return { background: "#dbeafe", color: "#1e6091", border: "1px solid #bfdbfe" };
    case PURPOSES.WORK:
      return { background: "#dcfce7", color: "#2d6a4f", border: "1px solid #bbf7d0" };
...
function StatusBadge({ status }) {
  ...
  return (
    <span style={{ fontSize: 12, fontWeight: 700, color: "#4d227c", fontFamily: "'Poppins', sans-serif" }}>
      {status || "—"}
    </span>
  );
}
      ...
      <div className={styles.pdfViewerModal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.pdfViewerHeader}>
          ...
          <div className={styles.pdfViewerActions}>
            ...
            <button className={styles.pdfViewerClose} onClick={onClose} aria-label="Close PDF">
              <FiX size={16} />
            </button>
          </div>
        </div>
        ...
        <div className={styles.pdfToolbar}>
          <div className={styles.pdfPagination}>
            <button className={styles.pdfNavBtn} onClick={() => setPageNumber((p) => Math.max(1, p - 1))} disabled={pageNumber <= 1} aria-label="Previous page">‹</button>
            <span className={styles.pdfPageInfo}>{numPages ? `${pageNumber} / ${numPages}` : "—"}</span>
            <button className={styles.pdfNavBtn} onClick={() => setPageNumber((p) => Math.min(numPages, p + 1))} disabled={!numPages || pageNumber >= numPages} aria-label="Next page">›</button>
          </div>
          <div className={styles.pdfZoom}>
            <button className={styles.pdfNavBtn} onClick={() => setScale((s) => Math.max(0.5, +(s - 0.1).toFixed(1)))} aria-label="Zoom out">−</button>
            <span className={styles.pdfPageInfo}>{Math.round(scale * 100)}%</span>
            <button className={styles.pdfNavBtn} onClick={() => setScale((s) => Math.min(2.5, +(s + 0.1).toFixed(1)))} aria-label="Zoom in">+</button>
          </div>
        </div>
      </div>
    </div>
...
function PdfFileChip({ label, filename }) {
  ...
  return (
    <>
      <button className={styles.pdfChip} onClick={() => setOpen(true)} aria-label={`View ${label} PDF`}>
...
// ── DRAWER ACCORDION ──────────────────────────────────────────────────────
function DrawerAccordion({ title, icon, children, defaultOpen = false, accentColor = "#4D227C" }) {
  ...
  return (
    <div className={styles.card}>
      ...
      >
        <span style={{ display: "flex", alignItems: "center", gap: 8, color: accentColor, flex: 1 }}>
          {icon}
          {title}
        </span>
        <FiChevronDown
      ...
      {isOpen && (
        <div className={styles.accordionBodyWrapper} style={{ borderTop: `2px solid ${accentColor}` }}>
...
const VawcSection = ({ appt }) => (
  <DrawerAccordion title="VAWC — Initial Complaint & Details" icon={<span className={styles.vawcDot} />} accentColor="#dc2626" defaultOpen={false}>
    ...
    <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #f0f0f0" }}>
      ...
      <InfoRow label="Types of Abuse" value="Physical, Emotional, Threatening" />
    ...
    <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #f0f0f0" }}>
      <div style={{ fontWeight: 700, color: "#4d227c", fontSize: 11, marginBottom: 8, fontFamily: "'Poppins', sans-serif" }}>Previous Assessments:</div>
      <div style={{ fontSize: 12, color: "#666", lineHeight: 1.7, fontFamily: "'Poppins', sans-serif" }}>
        <div>• January 15, 2026: Initial psychological assessment - Suspected PTSD and anxiety</div>
        <div>• February 20, 2026: Follow-up session - Progress noted with therapy</div>
...
const LegalSection = ({ appt }) => (
  <DrawerAccordion title="Adoption / Legal — Court-Related Assessment" accentColor="#8B4545" defaultOpen={false}>
    <ComplaintBox text={appt.initialComplaint} />
    <div className={styles.docsSection} style={{ marginTop: 12 }}>
      <div className={styles.docsSectionLabel}>Court Documents:</div>
      <div className={styles.docsChipRow}>
        <PdfFileChip label="Court Order" filename="Court_Order.pdf" />
        <PdfFileChip label="PACO Referral" filename="PACO_CSWD_Referral_Document.pdf" />
        <PdfFileChip label="Previous Assessment" filename="Previous_Psych_Report.pdf" />
...
=======
    <DrawerAccordion title="School / Academic — Learning Assessment" accentColor="#1e6091" defaultOpen={false}>
      <div className={styles.docsSection}>
...
const WorkSection = ({ appt }) => (
  <DrawerAccordion title="Work-Related — Occupational Health Assessment" accentColor="#2d6a4f" defaultOpen={false}>
    <div>
...
const PreEmploymentSection = ({ appt }) => (
  <DrawerAccordion title="Pre-Employment Psychological Assessment" accentColor="#92400e" defaultOpen={false}>
    ...
    <InfoRow label="Assessment Type" value="Comprehensive Pre-Employment Evaluation" />
    <InfoRow label="Tests Included" value="IQ, Personality, Clinical Screening" />
    <div style={{ marginTop: 12, paddingTop: 12, borderTop: "1px solid #f0f0f0" }}>
      <div style={{ fontWeight: 700, color: "#4d227c", fontSize: 11, marginBottom: 4, fontFamily: "'Poppins', sans-serif" }}>Assessment Results Status:</div>
      <div style={{ fontSize: 12, color: "#666", fontFamily: "'Poppins', sans-serif" }}>Pending final evaluation and report compilation</div>
    </div>
  </DrawerAccordion>
);
...
const ESASection = ({ appt }) => {
  ...
  return (
    <DrawerAccordion title="Emotional Support Animal (ESA) Assessment" accentColor="#0e7490" defaultOpen={false}>
      <InfoRow label="Place of Travel" value={
        <Badge style={isInternational
      ...
      <InfoRow label="Duration of Travel" value="3 months (June - August 2026)" />
      <InfoRow label="Type of Animal" value="Golden Retriever (Service Dog)" />
      <InfoRow label="Existing Clinical Diagnosis" value={
        <Badge style={appt.hasExistingDiagnosis
      ...
      {appt.hasExistingDiagnosis && hasDocs && (
        <div className={styles.docsSection} style={{ marginTop: 12 }}>
          <div className={styles.docsSectionLabel}><FiPaperclip size={12} style={{ marginRight: 5 }} />Medical Records &amp; Letters:</div>
          <div className={styles.docsChipRow}>
...
const InternshipSection = ({ appt }) => (
  <DrawerAccordion title="Mental Health for Internship" accentColor="#5b21b6" defaultOpen={false}>
    <InfoRow label="School / University" value={appt.schoolName || "Not specified"} />
...
const AssessmentSessionSection = ({ appt }) => {
  ...
  return (
    <DrawerAccordion title={session.sessionType} accentColor={badgeStyle.color} defaultOpen={false}>
        ...
        <>
          ...
          <div className={styles.certReminderBox}>
            ...
            <span className={styles.certReminderText}>
              The certificate must be claimed <strong>in person</strong> at the clinic. Please bring a valid government-issued ID upon release.
            </span>
          </div>
        </>
      )}
...
const RescheduleSection = ({ appt }) => (
  <DrawerAccordion title="Reschedule Status" accentColor="#b45309" defaultOpen={false}>
    <InfoRow label="Original Date" value="March 5, 2026 | 10:00 AM - 11:00 AM" />
    ...
    <InfoRow label="Reason for Reschedule" value={appt.rescheduledTo.reason || "Not specified"} />
...
<<<<<<< HEAD
=======
  const displayDateShort = format(new Date(appt.start), "MMMM dd, yyyy");
>>>>>>> c74f280d2ba6540f658a249cc8ef59ef48257997
      ...
      <div className={styles.overlay} onClick={onClose}>
        <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
          ...
          <div className={styles.modalHeader}>
            ...
            <div className={styles.modalHeaderRight}>
              <span className={styles.dateBadge}>{displayDateShort}</span>
              <button className={styles.closeBtn} onClick={onClose} aria-label="Close">
                <FiX />
              </button>
            </div>
            ...
            <section className={styles.card}>
              ...
              <div className={styles.cardBody}>
                <div className={styles.cardFields}>
                  <InfoRow label="Patient Name" value={appt.patientName || "—"} />
...
function DayAppointmentsModal({ isOpen, onClose, selectedDate, events, singleEventId = null }) {
  ...
  const formattedDateShort = format(selectedDateObj, "MMMM dd, yyyy");
  ...
  if (singleEventId && !activeAppt) {
    ...
    if (target)
      return <DetailView appt={target} isGhost={!!target.isRescheduledGhost} onClose={onClose} allEvents={events} />;
  }
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        <div className={styles.modalHeader}>
          ...
          <div className={styles.modalHeaderRight}>
            <span className={styles.dateBadge}>{formattedDateShort}</span>
            <button className={styles.closeBtn} onClick={onClose} aria-label="Close">
              <FiX />
            </button>
          </div>
        ...
        <div className={styles.scrollBody}>
          {dayAppointments.length === 0 ? (
            <div style={{
              ...
              <p style={{ margin: 0 }}>No appointments scheduled for this day.</p>
            </div>
          ) : (
<<<<<<< HEAD
            <div
                ...
                return (
                  <div key={appt.id} className={styles.card}>
                    ...
                    <div
                      ...
                      style={{
                        padding:
                          "clamp(12px, 2vw, 16px) clamp(14px, 2.5vw, 20px)",
                      }}
                    >
              ...
              return (
                <div key={appt.id} className={styles.card}>
                  <div className={styles.cardHeader}>
                    ...
                    <span className={styles.badgeGroup}>
                      {displayPurpose && (
                        <Badge style={{ ...purposeInlineStyle(displayPurpose), fontSize: 11, padding: "2px 10px" }}>
                          {displayPurpose}
>>>>>>> c74f280d2ba6540f658a249cc8ef59ef48257997
                        </Badge>
                      )}
                      {!displayPurpose && (isEsa || isIntern) && (
                        <Badge style={{ ...serviceTypeInlineStyle(svc), fontSize: 11, padding: "2px 10px" }}>
                          {isEsa ? "ESA" : "Internship"}
                        </Badge>
                      )}
