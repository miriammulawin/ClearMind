...
const PAaEAppointment = () => {
  ...
  useEffect(() => {
    axiosClient
      ...
      .then(({ data }) => {
        ...
        // Find the Psychological Assessment service
        const paService = services.find((s) =>
          s.service_name.toLowerCase().includes("psychological assessment"),
        );

        // Get active purposes
        const activePurposes = (paService?.purposes || []).filter(
          (p) => p.is_active === true || p.is_active === 1,
        );

        setPurposes(activePurposes);
      })
      .catch((e) => console.error("Fetch purposes error:", e))
  ...
  // Match purpose name to PAE_SERVICES title
  const getPurposeFee = (serviceTitle) => {
    const match = purposes.find(
      (p) => p.purpose_name.toLowerCase() === serviceTitle.toLowerCase(),
    );
    return match ? formatPrice(match.price) : null;
  };
  ...
  // Keep PAE_SERVICES as is — just add fee display
  const PAE_SERVICES = [
    {
      id: "0",
      title: "VAWC Purpose",
            ...
            {loading ? (
              ...
              <Accordion>
                {PAE_SERVICES.map((service) => {
                  const fee = getPurposeFee(service.title);
                  return (
                    <Accordion.Item
                      key={service.id}
                      eventKey={service.id}
                      ...
                      <Accordion.Body className={styles.accordionBody}>
                        ...
                        {fee && (
                          <div className={styles.servicePriceRow}>
                            ...
                            <span className={styles.priceValue}>{fee}</span>
                          </div>
                        )}
